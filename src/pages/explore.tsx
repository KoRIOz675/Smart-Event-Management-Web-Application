...
export default function ExplorePage() {
  ...
  const formatTime = (date: string) =>
    new Date(date).toLocaleTimeString(lang === 'fr' ? 'fr-FR' : 'en-GB', { hour: '2-digit', minute: '2-digit' });
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8">
          <div className="max-w-2xl">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-4">
              {e.title} <span className="text-primary">{e.titleHighlight}</span>
            </h1>
        ...
        {loading ? (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-5 border-t border-border flex justify-between items-center">
                    <div>
                      <span className="text-[10px] text-muted-foreground block uppercase font-black">{e.price}</span>
                      <span className="font-black text-lg">
                        {parseFloat(event.price as string) === 0 ? e.free : `${event.price}€`}
                      </span>
                    </div>
                    <button
          ...
          <div className="py-20 text-center bg-card rounded-radius-4xl border border-dashed border-border">
            ...
            <h3 className="text-xl font-bold">{e.noResults}</h3>
            <p className="text-muted-foreground">{e.noResultsDesc}</p>
          </div>
        )}
      </main>
